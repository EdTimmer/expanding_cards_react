    ...
    <div className="container">
      {
        panels.map((panel, index) => {
          const clName = `panel ${activeIndex === index ? 'active' : ''}` 
          return (
            <Panel background={panel.img} title={panel.title} key={index} className={clName} handleClick={handleClick} index={index}/>
          )
        })
      }
    </div>
