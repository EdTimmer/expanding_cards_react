...
const App = () => {
  ...
  const clName = `container active-${activeIndex}` 
  
  return (
    <div className={clName}>
      {
        panels.map((panel, index) => {
          const panelClName = `panel ${activeIndex === index ? 'active' : ''}` 
          return (
            <Panel background={panel.img} title={panel.title} key={index} className={panelClName} handleClick={handleClick} index={index}/>
          )
        })
      }
    </div>
